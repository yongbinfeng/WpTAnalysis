...
def main():
    ...
    parser.add_argument("--applyScaling", action="store_true", dest="applyScaling",
                        help="Scale the MC cross section by 30 percents and subtract")
    ...
    if applyScaling:
        ...
        mcscale = 1.3
    DataAisoSamp = Sample(input_antiiso_data, isMC=False, name="Data_aiso",
                          isWSR=True, additionalnorm=qcdnorm, legend='QCD', color='226')
    ...
    if not is5TeV:
        ...
        sampMan = SampleManager(DataAisoSamp, [Wl0AisoSamp, Wl1AisoSamp, Wl2AisoSamp, TTbarAisoSamp, TT1LepAisoSamp,
                                TT0LepAisoSamp, WWAisoSamp, WZAisoSamp, ZZAisoSamp, ZXXAisoSamp, Wx0AisoSamp, Wx1AisoSamp, Wx2AisoSamp])

        sampMan.groupMCs(["WW_aiso", "WZ_aiso", "ZZ_aiso", "ZXX_aiso",
                         "wx0_aiso", "wx1_aiso", "wx2_aiso"], "EWK", 216, "EWK")
        sampMan.groupMCs(["ttbar_dilepton_aiso", "ttbar_1lepton_aiso",
                         "ttbar_0lepton_aiso"], "ttbar", 96, "t#bar{t}")
        label = "W#rightarrow#mu#nu" if doMuon else "W#rightarrow e#nu"
        sampMan.groupMCs(
            ['wl0_aiso', 'wl1_aiso', 'wl2_aiso'], "wlnu", 92, label)

    else:
        # W -> lnu
        label = "W#rightarrow#mu#nu" if doMuon else "W#rightarrow e#nu"
        WlAisoSamp = Sample(input_antiiso_wl, isMC=True, name="wlnu", isWSR=True, additionalnorm=qcdnorm *
                            mcscale, is5TeV=True, color=92, legend=label, doTheoryVariation=False)
        # ttbar
        TTbarAisoSamp = Sample(input_antiiso_ttbar, isMC=True, name="ttbar",     isWSR=True,
        ...
        sampMan = SampleManager(DataAisoSamp, [WlAisoSamp, TTbarAisoSamp, WWAisoSamp,
                                WZAisoSamp, ZZ2LAisoSamp, ZZ4LAisoSamp, ZXXAisoSamp, WxAisoSamp], is5TeV=True)

        sampMan.groupMCs(["WW_aiso", "WZ_aiso", "ZZ2L_aiso",
                         "ZZ4L_aiso", "ZXX_aiso", "wx_aiso", ], "EWK", 216, "EWK")
    ...
    if doMuon:
        isoCuts = [0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50,
    ...
    sampMan.DefineMC(
        "met_phi", "TVector2::Phi_mpi_pi(metVarsPhi[1])", excludes=['Data_aiso'])
        ...
        for wpt in wptbins:
            for lepeta in etabins:
                for chg in chgbins:
                    ...
                    sampMan.cacheDraw("mtCorr", outputname, mass_bins, DrawConfig(xmin=xmin, xmax=xmax, xlabel="m_{T} [GeV]", ylabel=f"Events / {(xmax-xmin)/nbins:.0f} GeV", dology=True, ymax=6e5, donormalizebin=False,
                                      addOverflow=True, addUnderflow=True, showratio=False, lheader=f"{isoCuts[idx]} < I < {isoCuts[idx + 1]}", legendPos=[0.94, 0.88, 0.70, 0.68], legends=legends.copy()), weightname=strname)
